...
function fetchData() {
    //$('#loading2').show();
    if (lastdoc) { var query = db.collection("vod").orderBy('time', 'desc').startAfter(lastdoc).limit(10); console.log("lastdoc exists"); }
    else
        var query = db.collection("vod").orderBy('time', 'desc').limit(10);
...
function addcomment(evt, id) {
    ...
    db.collection("vod").doc(id).update({ comments: firebase.firestore.FieldValue.arrayUnion({ comment: commentin, user: username }) });

    db.collection("vod").doc(id).get().then(function (doc) { updateuI(doc) });
}
